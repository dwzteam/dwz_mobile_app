	...
	$.fn.extend({
		disableAutofocus: function(){
			return this.each(function(){
				...
				setTimeout(function(){$input.removeAttr('readonly');}, 600);
			});
		},
		redirect: function(){
			return this.each(function(){
		...
		activeClass: function(className){
			...
			return this.each(function(){
				...
				if ($.event.hasTouch) {
					$this.on('touchstart', function(){ $this.addClass(className); });
					$this.on('touchend', function(){ $this.removeClass(className); });
				} else {
					$this.on('mousedown', function(){ $this.addClass(className); });
		...
		touchOpenRight: function (option) {
			var op = $.extend({ctlClass: 'dwz-open-right', openClass: 'open-right'});
		...
		toggleSelectRef: function(options){
			...
			return this.each(function(){
				...
				function _toggleAll(){
					...
					var ignoreOnly = $select.hasClass('ignoreOnly');
