...
dwz.plus = {
	...
	captureImage: function(options){
		var op = $.extend({maxWidth:1280, maxHeight:1280, callback: null, miscCallback: null}, options);

		var me = this;
		var cmr = plus.camera.getCamera();
        cmr.captureImage(function(path) {
            plus.io.resolveLocalFileSystemURL(path, function(entry) {
	            if (op.miscCallback) { // 使用miscCallback时需要自己处理，忽略maxWidth, maxHeight, callback
	...
	galleryImage: function(options){
		var op = $.extend({maximum:1, maxWidth:1280, maxHeight:1280, callback: null, miscCallback: null}, options);
		var me = this;
		plus.gallery.pick(
			function(e){
				var arrayBase64 = [];
    ...
    getBase64Image: function(options){
		var op = $.extend({imgPath:'', maxWidth:1280, maxHeight:1280, callback: null}, options);
		var img = new Image();
        img.src = op.imgPath;
        img.onload = function() {
            ...
            var strBase64 = canvas.toDataURL("image/png", 0.5);
...
function previewUploadImg(previewElem, strBase64, inputName, maxW, maxH) {
	...
	var inputElem = dwz.parseHTML('<textarea style="display:none" name="'+inputName+'">'+strBase64+'</textarea>');
...
$.fn.extend({
	...
	previewUploadImg: function(options){
		var op = $.extend({maxW:80, maxH:80, maxCount:4, inputName:'pics[]'}, options);
