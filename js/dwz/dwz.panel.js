 ...
 */
(function($){
    var _config = {panelCentent$:'.panel-content', collapseBtn$:'.panel-title-right', iconUpClass:'icon-list-u'};
    ...
    $.altPanel = {
        ...
        open: function(options) {
            // default, pic, login
            var op = $.extend({type:'GET', url:'', data:{}, callback: null, pos:null}, options);
            ...
            if (op.pos) {
                ...
                if (op.pos.y < screenH/2){
                    $box.css({top:(op.pos.y+5)+'px', bottom:'auto'}).removeClass('pop-up');
                } else {
                    $box.css({top:'auto', bottom:(screenH-op.pos.y+5)+'px'}).addClass('pop-up');
                }
            }
