...
(function($){
	$.croppic = {
		...
		imgCropData: function($box, options){
			var op = $.extend({imgBg$:'img.croppic-bg', imgWrap$:'.crop-img-wrap'}, options);
			...
			if (data.width>data.naturalWidth-data.left) {data.width = data.naturalWidth-data.left;}
			if (data.height>data.naturalHeight-data.top) {data.height = data.naturalHeight-data.top;}
		...
		render: function ($box, options) {
			...
			var zoomUtil = {
				...
				setZoom: function(eventPos) {
					var distanceStart = $.event.getDistance(eventPos.startX, eventPos.startY, eventPos.startX2, eventPos.startY2);
					var distance = $.event.getDistance(eventPos.x, eventPos.y, eventPos.x2, eventPos.y2);
					...
					if (width < $imgWrap.width()) {width = $imgWrap.width();}
